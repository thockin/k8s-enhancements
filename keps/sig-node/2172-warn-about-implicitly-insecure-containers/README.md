...
- [Proposal](#proposal)
  ...
  - [CLI warnings when running implicitly-root](#cli-warnings-when-running-implicitly-root)
    - [Color](#color)
  - [Metrics](#metrics)
...
Some CVEs around containerd come down to containers running as root when they
...
3) To make noise about explicitly-root containers.

## Proposal

This proposal includes several parts.  It depends on KEP #3619 "Fine-grained
SupplementalGroups control", which add the running UID and GID to Pods' status.

### Condition when running implicitly-root

Whenever kubelet sees an implicitly-root container, it will add a condition -
"InsecureUserID" or "InsecureGroupID" for UIDs and GIDs, respectively - to the
pod. Users who see these can look at the `Pod.status.containerStatuses[]` for
more details.
...
These events will be bypassed if the user explicitly sets `runAsUser` or
`runAsGroup` in their pod.

### CLI warnings when running implicitly-root
...
Kubelet will add metrics indicating the number of pods with implicitly-root and
explicitly-root containers.
...
see any pods that are implicitly-root.
